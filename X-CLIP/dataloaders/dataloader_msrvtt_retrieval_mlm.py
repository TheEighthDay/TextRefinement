    ...
    def _get_maskinputid_masklabel(self, video_id, caption=None):
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if self.mask_mode == "random":
                ...
                not_selection = torch.flatten(not_mask_arr.nonzero()).tolist()
                ...
                rand = torch.tensor([ 0 if i%2==0 else 1 for i in range(32)]) #mask掉奇数位置
...
class MSRVTT_TrainDataLoader_MLM(Dataset):
    ...
    def _get_maskinputid_masklabel_saliency(self, video_id, caption=None):
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if self.mask_mode == "random":
                rand = torch.rand(input_ids.shape)
                mask_arr = (rand < 0.3) * (input_ids != 49406) * (input_ids != 49407) * (input_ids != 0) * (saliency_ids!=0)
                not_mask_arr = ~mask_arr
...
class MSRVTT_TrainDataLoader_Gen(Dataset):
    ...
    def _get_maskinputid_masklabel(self, video_id, caption=None):
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if self.mask_mode == "random_add":
            ...
            elif self.mask_mode=="random_replace":
                ...
                input_ids[rand_index] = 49339
                input_label[rand_index] = -100 
