...
from modules.mlm_head import BertOnlyMLMHead
...
def cross_attn(q, k, v):
    ...
    attn_map = rearrange(attn, '(b h) ntext (f nvideo) -> b h f ntext nvideo', f=12,h=8)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, question, mask, whether, einops_from, einops_to, **einops_dims):
        ...
        # print("k.shape",k.shape) #k,v [16, 12x49, 64]
        # print("v.shape",k.shape)
        # print("q_.shape",q_.shape) #q_ [16, 31, 64]
        

        # k, v = map(lambda t: rearrange(t, f'{einops_from} -> {einops_to}', **einops_dims), (k, v)) #'b (f n) d'  '(b f) n d'
        ...
        # cls_q.shape torch.Size([16, 1, 64])  
        # 把attn勾出来

        #out1.shape torch.Size([192, 31, 64])
        #out2.shape torch.Size([16, 372, 64])
...
class CrossAttention2(nn.Module):
    ...
    def forward(self, x, question, mask):
        ...
        out = attn_mask(q, k, v,mask)

        # print("out1.shape2",out.shape)
        # out = rearrange(out, f'{einops_to} -> {einops_from}', **einops_dims)
        # print("out2.shape2",out.shape)

        # concat back the cls token
        # out = torch.cat((cls_out, out), dim=1)
        # print("out3.shape2",out.shape)

        # merge back the heads
        out = rearrange(out, '(b h) n d -> b n (h d)', h=h)
        # print("out4.shape2",out.shape)
...
class Video_Bridge_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.bridgeattn = CrossAttention2(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x_bridge, x, question, mask, layer, einops_from_space, einops_to_space,
        ...
        space_bridge_output = self.bridgeattn(self.norm7(bridge_temp),self.norm7(bridge_temp),mask)
...
class Single_Tower(nn.Module):
    def __init__(self, img_size=224, patch_size=32, num_classes=0, embed_dim=512, depth=12, #12
        ...
        self.head = BertOnlyMLMHead(hidden_size=self.embed_dim,vocab_size=num_classes) if num_classes > 0 else nn.Identity()
