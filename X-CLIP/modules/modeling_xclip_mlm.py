...
from modules.module_clip_bridge import CLIP, convert_weights
from modules.modeling import CLIP4ClipPreTrainedModel, show_log, update_attr, check_attr

from modules.bridge_former_mlm import Single_Tower
...
class XCLIPMLM(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        super(XCLIPMLM, self).__init__(cross_config)
        ...
        self.loss_mlm_fun = nn.CrossEntropyLoss()

        self.bridge = Single_Tower(img_size=image_resolution,patch_size=vision_patch_size,num_frames = self.task_config.max_frames,num_classes=49408)

        self.apply(self.init_weights)

    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,masked_input_label=None,novelty=False):
        ...
        # [bs, 1, dim], [bs, num_words, dim], [bs, num_frames, dim], [bs, num_framesxpatches, dim],
        ...
        (sequence_output, seq_features,levels_sequence_output,levels_seq_features), (visual_output,visual_patches,levels_visual_output,levels_visual_patches)= self.get_sequence_visual_output(input_ids, token_type_ids, attention_mask, 
                                                                video, video_mask, shaped=True, video_frame=video_frame)


        # attention_mask shape: torch.Size([B, 32])                                                                                                                                                                          
        # levels_seq_features[0] shape: torch.Size([B, 32, 512])                                                                                                                                                             
        # levels_visual_patches[0] shape: torch.Size([B, 12x49, 768]) 

        seq_vis_features = self.bridge(levels_visual_patches,levels_seq_features,attention_mask)
        # print("seq_vis_features.shape",seq_vis_features.shape)
        # seq_vis_features.shape torch.Size([B, 373?, 512]) 
        #seq_vis_features shape torch.Size([B, 9648])  
        masked_input_label = masked_input_label.float().to(visual_output.device)

        if self.training:
            if not novelty:
                loss = 0.
                masked_input_label = allgather(masked_input_label, self.task_config)
                seq_vis_features = allgather(seq_vis_features, self.task_config)
                torch.distributed.barrier()
                batch_size= seq_vis_features.shape[0]
                
                loss_mlm=0.0

                for i in range(batch_size):

                    tmp_seq_vis_features = seq_vis_features[i]
                    tmp_masked_input_label = masked_input_label[i].long()

                    tmpmlmloss = self.loss_mlm_fun(tmp_seq_vis_features,tmp_masked_input_label)
                    loss_mlm = loss_mlm+tmpmlmloss


                # loss_concrpt = self.loss_concrpt(seq_vis_features,masked_input_label)
                # print("loss_concrpt",loss_concrpt)
                loss = loss + loss_mlm
                ...
                seq_vis_features = allgather(seq_vis_features, self.task_config)
                torch.distributed.barrier()
                batch_size,max_words,dim= seq_vis_features.shape
                ...
                for i in range(batch_size):
                    ...
                    simmat = torch.matmul(tmp_seq_vis_features, tmp_seq_vis_features.t())
                    simlabel = torch.eye(max_words).float().to(simmat.device)

                    sim_loss1 = self.loss_fct(simmat)
                    # print(sim_loss1)
                    sim_loss2 = self.loss_fct(simmat.T)
                    sim_loss = (sim_loss1 + sim_loss2) / 2
                    
                    loss_mlm = loss_mlm+tmpmlmloss
                    loss_novelty = loss_novelty+sim_loss


                # loss_concrpt = self.loss_concrpt(seq_vis_features,masked_input_label)
                # print("loss_concrpt",loss_concrpt)
                loss = loss_novelty + loss_mlm
                return loss,loss_mlm
    ...
    def get_sequence_output(self, input_ids, token_type_ids, attention_mask, shaped=False):
        ...
        sequence_hidden, seq_features,levels_sequence_hidden,levels_seq_features = self.clip.encode_text(input_ids, return_hidden=True)
        sequence_hidden, seq_features = sequence_hidden.float(), seq_features.float()
        sequence_hidden = sequence_hidden.view(bs_pair, -1, sequence_hidden.size(-1))

        return sequence_hidden, seq_features,levels_sequence_hidden,levels_seq_features
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        visual_hidden,visual_patchesplusclas,levels_visual_hidden,levels_visual_patchesplusclas = self.clip.encode_image(video, video_frame=video_frame,return_hidden=True)

        visual_hidden = visual_hidden.float()
        visual_patchesplusclas = visual_patchesplusclas.float()

        visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))
        for i in range(len(levels_visual_hidden)):
            #torch.Size([600, 768])   --> torch.Size([50, 12, 768])  
            levels_visual_hidden[i] = levels_visual_hidden[i].view(bs_pair, -1, levels_visual_hidden[i].size(-1))

        visual_patches = visual_patchesplusclas[:,1:,:]
        visual_patches = rearrange(visual_patches, '(b f) n d -> b (f n ) d', b=bs_pair)

        for i in range(len(levels_visual_patchesplusclas)):
            levels_visual_patchesplusclas[i] = levels_visual_patchesplusclas[i][:,1:,:]
            levels_visual_patchesplusclas[i] = rearrange(levels_visual_patchesplusclas[i], '(b f) n d -> b (f n ) d', b=bs_pair)

        levels_visual_patches = levels_visual_patchesplusclas

        return visual_hidden,visual_patches,levels_visual_hidden,levels_visual_patches
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False, video_frame=-1):
        ...
        sequence_output, seq_features,levels_sequence_output,levels_seq_features = self.get_sequence_output(input_ids, token_type_ids, attention_mask, shaped=True) # [bs, 1, dim], [bs, num_words, dim]
        visual_output, visual_patches,levels_visual_output,levels_visual_patches = self.get_visual_output(video, video_mask, shaped=True, video_frame=video_frame)                  # [bs, num_frames, dim]

        return (sequence_output, seq_features,levels_sequence_output,levels_seq_features), (visual_output,visual_patches,levels_visual_output,levels_visual_patches)
