...
class XCLIP(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,multi_augmentation=False):
        ...
        if multi_augmentation:
            (sequence_output_list, seq_features_list), visual_output = self.get_sequence_visual_output(input_ids, token_type_ids, attention_mask, 
                                                                    video, video_mask, shaped=True, video_frame=video_frame,multi_augmentation=True)
            if self.training:
                loss = 0.
                all_sim_matrix = []
                for i in range(len(sequence_output_list)):
                    tmp_sim_matrix, *_tmp = self.get_similarity_logits(sequence_output_list[i], seq_features_list[i], visual_output, attention_mask, 
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False, video_frame=-1,multi_augmentation=False):
        if multi_augmentation:
            ...
            return (sequence_output_list, seq_features_list), visual_output
