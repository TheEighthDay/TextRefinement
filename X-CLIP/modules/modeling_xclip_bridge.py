...
from modules.bridge_former import Single_Tower
...
class XCLIPBridge(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        super(XCLIPBridge, self).__init__(cross_config)
        ...
        self.loss_concrpt = nn.BCEWithLogitsLoss()

        self.bridge = Single_Tower(num_frames = 12,num_classes=9648)

        self.apply(self.init_weights)

    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,concept_label_onehot=None):
        ...
        concept_label_onehot = concept_label_onehot.float().to(visual_output.device)
        ...
        if self.training:
            ...
            # print("loss",loss)
            if self.training:
                concept_label_onehot = allgather(concept_label_onehot, self.task_config)
                seq_vis_features = allgather(seq_vis_features, self.task_config)
                torch.distributed.barrier()
            loss_concrpt = self.loss_concrpt(seq_vis_features,concept_label_onehot)
            # print("loss_concrpt",loss_concrpt)
            loss = loss + loss_concrpt

            return loss,loss_concrpt
        else:
            return None
