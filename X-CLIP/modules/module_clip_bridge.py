...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask=None):
        ...
        for i in range(self.layers):
            ...
            multi_level.append(x)
        return multi_level[-1],multi_level[:-1]
        # return res[-1]
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1):
        ...
        x,multi_level = self.transformer(x, attn_mask=None)
        x = x.permute(1, 0, 2)  # LND -> NLD
        for i in range(len(multi_level)):
            multi_level[i] = multi_level[i].permute(1, 0, 2).float()
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1):
        hidden,multi_level = self.visual(image.type(self.dtype), video_frame=video_frame)
        hidden = self.visual.ln_post(hidden) @ self.visual.proj

        # vis multi_level[0] shape torch.Size([600, 50, 768])                                                                                                                                                                 
        # vis multi_level[1] shape torch.Size([600, 50, 768])  
        x = hidden[:, 0, :]
        # [BxF,cls_toen,D]
        level_x = []
        for i in range(len(multi_level)):
            level_x.append(multi_level[i][:, 0, :])

        if return_hidden:
            return x, hidden,level_x,multi_level
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        x = x + pos_emd
        x = x.permute(1, 0, 2)  # NLD -> LND
        x,multi_level = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD

        for i in range(len(multi_level)):
            multi_level[i] = multi_level[i].permute(1, 0, 2).float()
        
        # text multi_level[0] shape torch.Size([50, 32, 512])                                                                                                                                                                 
        # text multi_level[1] shape torch.Size([50, 32, 512])  
        ...
        level_x = []
        for i in range(len(multi_level)):
            level_x.append(multi_level[i][torch.arange(hidden.shape[0]), text.argmax(dim=-1)])
