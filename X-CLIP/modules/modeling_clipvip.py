...
from modules.module_clip_vip import CLIP, convert_weights
...
class CLIPVIP(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        super(CLIPVIP, self).__init__(cross_config)
        ...
        show_log(task_config, "CLIP start vip : {}".format(self.task_config.vip_start))
        self.clip = CLIP(
            embed_dim,
            image_resolution, vision_layers-cut_top_layer, vision_width, vision_patch_size,
            context_length, vocab_size, transformer_width, transformer_heads, transformer_layers-cut_top_layer,
            linear_patch=self.linear_patch, vip_start=self.task_config.vip_start
        ).float()
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        # input_ids.shape torch.Size([10, 32])                                                                                                                
        # token_type_ids.shape torch.Size([10, 32])                                                                                                           
        # attention_mask.shape torch.Size([10, 32])                                                                                                           
        # video_mask.shape torch.Size([10, 12])                                                                                                               
        # video.shape torch.Size([10, 1, 12, 1, 3, 224, 224])                                                                                                 
        # video.shape torch.Size([120, 3, 224, 224]) 

        # T x 3 x H x W
        video = torch.as_tensor(video).float()
        b, pair, bs, ts, channel, h, w = video.shape
        # Batch, 1, 12, 1, 3, 224, 224
        # bs ts 是为 multiview multicrop设计的
        video = video.view(b * pair * bs * ts, channel, h, w)
        video_frame = bs * ts

        if self.task_config.vip_start:
            # [bs, 1, dim], [bs, num_words, dim], [bs, num_frames, dim]
            (sequence_output, seq_features), (visual_output,visual_vip) = self.get_sequence_visual_output(input_ids, token_type_ids, attention_mask, 
                                                                    video, video_mask, shaped=True, video_frame=video_frame)

            if self.training:
                loss = 0.
                sim_matrix, *_tmp = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
                                            video_mask, shaped=True, loose_type=self.loose_type,visual_vip=visual_vip)
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        if self.task_config.vip_start:
            visual_hidden,visual_vip = self.clip.encode_image(video, video_frame=video_frame)
            visual_hidden = visual_hidden.float()
            visual_vip = visual_vip.float()
            # [batch_pair_frame,width],[batch_pair,width]
            visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))
            return visual_hidden,visual_vip
        else:
            visual_hidden = self.clip.encode_image(video, video_frame=video_frame).float()
            # [batch_pair_frame,width]
            visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False, video_frame=-1):
        if shaped is False:
            ...
            video = video.view(b * pair * bs * ts, channel, h, w)
            video_frame = bs * ts
        if not self.task_config.vip_start:
            ...
            sequence_output, seq_features = self.get_sequence_output(input_ids, token_type_ids, attention_mask, shaped=True) # [bs, 1, dim], [bs, num_words, dim]
            visual_output,visual_vip = self.get_visual_output(video, video_mask, shaped=True, video_frame=video_frame)                  # [bs, num_frames, dim]
            return (sequence_output, seq_features), (visual_output,visual_vip)
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header="meanP",visual_vip=None):
        """
        ...
        if sim_header == "meanP":
            ...
            # extended_video_mask.shape torch.Size([10, 12, 12])
            # visual_output.shape torch.Size([10, 12, 512])
        ...
        if self.task_config.vip_start:
            # 这里我们将video_output定义为visual_vip
            visual_vip = visual_vip/visual_vip.norm(dim=-1, keepdim=True)
            video_output = visual_vip
        else:
    ...
    def get_similarity_logits(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, shaped=False, loose_type=False, visual_vip=None):
        if shaped is False:
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        ...
        if loose_type:
            ...
            if self.task_config.vip_start:
                retrieve_logits = self._loose_similarity(sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header=self.sim_header,visual_vip=visual_vip)
            else:
