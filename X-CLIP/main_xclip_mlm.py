...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        input_ids, input_mask, segment_ids, video, video_mask, masked_input_ids,masked_input_mask,masked_segment_ids,masked_input_label = batch
    ...
    #     # video_mask.shape ([50, 1, 12]) b, pair, frame
        ...
        loss,loss_concrpt = model(masked_input_ids, masked_segment_ids, masked_input_mask, video, video_mask, masked_input_label,novelty=True)
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    np.save(os.path.join(args.output_dir,"mask_probs.npy"),mask_probs)
    np.save(os.path.join(args.output_dir,"mask_labels.npy"),mask_labels)
    np.save(os.path.join(args.output_dir,"attens.npy"),attens)
    return Acc
...
def main():
    ...
    if DATALOADER_DICT[args.datatype]["test_mlm"] is not None:
        test_dataloader, test_length = DATALOADER_DICT[args.datatype]["test_mlm"](args, tokenizer)

    if DATALOADER_DICT[args.datatype]["val_mlm"] is not None:
        val_dataloader, val_length = DATALOADER_DICT[args.datatype]["val_mlm"](args, tokenizer, subset="val_mlm")
    else:
    ...
    if args.do_train:
        train_dataloader, train_length, train_sampler = DATALOADER_DICT[args.datatype]["train_mlm"](args, tokenizer)
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            if args.local_rank == 0:
                ...
                Acc = eval_epoch(args, model, val_dataloader, device, n_gpu)
